...
3. **`az login`**，且账号对组织 **`slb1-swt`** 及目标 **Artifacts feed**（见 **`scripts/shared-package-feed-config.json`** 中的 `feedName`，当前为 **PrismLibrary**；与 **PrismService** 上同一包 ID 的最新版可能不同）有读权限。
...
- 配置见 **`scripts/shared-package-feed-config.json`**（`organizationUrl`、`feedName`）。
